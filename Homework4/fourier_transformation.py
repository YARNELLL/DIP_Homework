...
if __name__ == '__main__':
    ...
    # output_img_Sample_inverse = inverse_fourier_transformation(output_img_Sample)

    output_img_Lena = np.fft.fft2(img_Lena)
    output_img_Lena = np.fft.fftshift(output_img_Lena)
    output_img_Lena_inverse = np.fft.ifftshift(output_img_Lena)
    output_img_Lena_inverse = np.fft.ifft2(output_img_Lena_inverse)

    output_img_Sample = np.fft.fft2(img_Sample)
    output_img_Sample = np.fft.fftshift(output_img_Sample)
    output_img_Sample_inverse = np.fft.ifftshift(output_img_Sample)
    output_img_Sample_inverse = np.fft.ifft2(output_img_Sample_inverse)
    # 存储图片
    cv2.imwrite("./output/output_img_Lena.png", np.uint16(np.abs(output_img_Lena)))
    cv2.imwrite("./output/output_img_Sample.png", np.uint16(np.abs(output_img_Sample)))
    cv2.imwrite("./output/output_img_Lena_inverse.png", np.abs(output_img_Lena_inverse))
    cv2.imwrite("./output/output_img_Sample_inverse.png", np.abs(output_img_Sample_inverse))
